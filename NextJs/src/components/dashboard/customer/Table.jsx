...
const Table = ({ customers = [], edit = false, remove = false }) => {
  const token = window.localStorage.getItem('token')
  const deleteCustomer = async (id) => {
    const deleteToast = toast.loading('Delete Customer...')
    const config = {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    }
    await axios.delete(`/admin/customer/delete/${id}`, config).then(() => {
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white">
              {customers.map((customer, index) => (
                <tr className="text-gray-700" key={customer.id}>
