...
const Table = ({
  ...
  publisherEmail = 'publisher@publisher.com',
  status = 'status',
  edit = false,
  remove = false,
}) => {
  ...
  const deletePublisher = async (id) => {
    const deleteToast = toast.loading('Delete publisher...')
    const config = {
      headers: {
    ...
    await axios
      .delete(`/admin/publisher/delete/${id}`, config)
      .then(() => {
          ...
          <table className="w-full">
            ...
            <tbody className="bg-white">
              {publishers.map((publisher, index) => (
                <tr className="text-gray-700" key={publisher.id}>
